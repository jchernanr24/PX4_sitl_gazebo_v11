...
#include "gazebo_motor_checks.h"
#include <ignition/math.hh>

namespace gazebo {

GazeboMotorChecks::~GazeboMotorChecks() {
  updateConnection_->~Connection();
  use_pid_ = false;
}

void GazeboMotorChecks::InitializeParams() {}
...
void GazeboMotorChecks::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("robotNamespace"))
    ...
    gzerr << "[gazebo_motor_checks] Please specify a robotNamespace.\n";
  ...
  if (_sdf->HasElement("jointName"))
    ...
    gzerr << "[gazebo_motor_checks] Please specify a jointName, where the rotor is attached.\n";
  // Get the pointer to the joint.
  joint_ = model_->GetJoint(joint_name_);
  if (joint_ == NULL)
    gzthrow("[gazebo_motor_checks] Couldn't find specified joint \"" << joint_name_ << "\".");
  ...
  if (_sdf->HasElement("linkName"))
    ...
    gzerr << "[gazebo_motor_checks] Please specify a linkName of the rotor.\n";
  link_ = model_->GetLink(link_name_);
  if (link_ == NULL)
    gzthrow("[gazebo_motor_checks] Couldn't find specified link \"" << link_name_ << "\".");


  if (_sdf->HasElement("motorNumber"))
    motor_number_ = _sdf->GetElement("motorNumber")->Get<int>();
  else
    gzerr << "[gazebo_motor_checks] Please specify a motorNumber.\n";
  ...
  if (_sdf->HasElement("turningDirection")) {
    ...
    if (turning_direction == "cw")
      ...
      gzerr << "[gazebo_motor_checks] Please only use 'cw' or 'ccw' as turningDirection.\n";
  }
  else
  ...
  */
  ...
  updateConnection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&GazeboMotorChecks::OnUpdate, this, _1));

  command_sub_ = node_handle_->Subscribe<mav_msgs::msgs::CommandMotorSpeed>("~/" + model_->GetName() + command_sub_topic_, &GazeboMotorChecks::VelocityCallback, this);
...
void GazeboMotorChecks::OnUpdate(const common::UpdateInfo& _info) {
...
void GazeboMotorChecks::VelocityCallback(CommandMotorSpeedPtr &rot_velocities) {
...
void GazeboMotorChecks::MotorFailureCallback(const boost::shared_ptr<const msgs::Int> &fail_msg) {
  motor_Failure_Number_ = fail_msg->data();
}

void GazeboMotorChecks::UpdateForcesAndMoments() {
  ...
  // link_->AddRelativeForce(ignition::math::Vector3d(0, 0, force * scalar)); // McFoamy
  ...
  // link_->AddForce(air_drag); //McFoamy
  // Moments
  ...
  // parent_links.at(0)->AddRelativeTorque(drag_torque_parent_frame); // McFoamy

  ignition::math::Vector3d rolling_moment;
  // - \omega * \mu_1 * V_A^{\perp}
  rolling_moment = -std::abs(real_motor_velocity) * rolling_moment_coefficient_ * body_velocity_perpendicular;
  // parent_links.at(0)->AddTorque(rolling_moment); //McFoamy
...
void GazeboMotorChecks::UpdateMotorFail() {
